...
const actions = {
		...
		globalAxios({
			...
			.then((res) => {
				...
				rootState.backendMessages.push({
					...
					msg: `Successfully fetched ${rootState.isUserRemoved ? 'removed' : ''} user '${payload.userName}'`,
				})
	 ...
	 */
	getAllUsers({ rootState, state, dispatch }) {
		rootState.backendMessages = []
		rootState.isUserFound = false
		state.allUsers = []
		globalAxios({
			...
			.then((res) => {
				...
				globalAxios({
					...
					.then(() => {
						rootState.backendMessages.push({
							seqKey: rootState.seqKey++,
							msg: `changeMyPasswordAction: The profile of user '${userData.name}' is updated successfully`,
						})
	...
	assignProductToUserAction({ rootState, dispatch }, payload) {
		globalAxios({
			method: 'GET',
			...
			.then((res) => {
				...
				dispatch('updateUserDb', {
					...
					onSuccessCallback: () => {
						...
						const msg =
							rolesSet.length === 0
								? `assignProductToUserAction: The product with Id ${productId} is added to your profile with no roles set`
	...
	changeCurrentDb({ rootState, dispatch }, payload) {
		globalAxios({
			method: 'GET',
			url: payload.dbName + '/_design/design1/_view/products',
		})
	 ...
	 */
	changeDbInMyProfileAction({ rootState, rootGetters, dispatch, commit }, payload) {
	...
	createUserIfNotExistentAction({ rootState, dispatch }, userData) {
		rootState.backendMessages = []
		globalAxios({
			method: 'GET',
			url: '/_users/org.couchdb.user:' + userData.name,
	...
	removeUserIfExistent({ rootState, dispatch }, userName) {
		rootState.backendMessages = []
		globalAxios({
			method: 'GET',
	...
	removeUserAction({ rootState, dispatch }, data) {
	...
	createUserAction({ rootState, dispatch }, userData) {
	...
	saveMyOptionsAsync({ rootState, dispatch, commit }) {
		globalAxios({
			...
			.then((res) => {
				...
				dispatch('updateUserDb', {
					data: tmpUserData,
					onSuccessCallback: () => commit('addToEventList', { txt: 'Your options have been saved', severity: SEV.INFO }),
				})
			})
			.catch((error) => {
	...
	saveMyTreeViewAsync({ rootState, dispatch, commit }, payload) {
		commit('createTreeExpansionState')
		globalAxios({
			method: 'GET',
			url: `/_users/org.couchdb.user:${rootState.userData.user}`,
		})
