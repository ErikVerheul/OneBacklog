						...
						switch (histEvent) {
							...
							case 'createEvent':
								if (node === null) {
									...
									}, [newNode], true, rootState.c_treeNodes)
								}
								break
							...
							case 'updateParentHistEvent':
								if (doc.delmark) {
									...
									// ToDo: Fix this --> window.slVueTree.correctDependencies(lastHistObj.updateParentHistEvent[0], lastHistObj.updateParentHistEvent[1])
									if (node) {
										window.slVueTree.remove([node])
										if (lastHistObj.by === rootState.userData.user) {
