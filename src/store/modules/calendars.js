...
const actions = {
	extendDefaultSprintCalendar({ rootState, dispatch }, payload) {
	...
	fetchDefaultSprintCalendarAction({ rootState, dispatch }, payload) {
			...
			.then((res) => {
				if (res.data.defaultSprintCalendar) {
					...
					rootState.backendMessages.push({
						...
						msg: `fetchDefaultSprintCalendarAction: success, ${res.data.defaultSprintCalendar.length} sprint periods are read`,
					})
	...
	saveDefaultSprintCalendarAction({ rootState, dispatch }, payload) {
		globalAxios({
			method: 'GET',
			url: payload.dbName + '/config',
		})
			.then((res) => {
				...
				dispatch('updateDoc', {
					...
					onSuccessCallback: () => {
						...
						rootState.backendMessages.push({
							seqKey: rootState.seqKey++,
							msg: `saveDefaultSprintCalendarAction: success, calendar with ${payload.newSprintCalendar.length} sprint periods is saved`,
						})
	...
	fetchTeamCalendarAction({ rootState, dispatch }, payload) {
		...
		globalAxios({
			...
			.then((res) => {
				if (res.data.teamCalendar) {
					...
					rootState.backendMessages.push({
						seqKey: rootState.seqKey++,
						msg: `fetchTeamCalendarAction: success, ${res.data.teamCalendar.length} sprint periods are read`,
					})
	 ...
	 */
	loadTeamCalendarAtStartup({ rootState, dispatch }, id) {
		globalAxios({
			method: 'GET',
	...
	/* For use in the startup sequence only. */
	extendTeamCalendar({ rootState, dispatch }, doc) {
