...
const actions = {
	 ...
	 */
	cloneBranch({ rootState, dispatch, commit }, originalNode) {
		if (busyCloning) {
		...
		globalAxios({
			...
			.then((res) => {
				...
				dispatch('processItemsToClone', {
					...
					cloneParentId: doc.parentId,
					cloneParentNode: undefined,
				})
			})
			.catch((error) => {
	...
	processItemsToClone({ rootState, dispatch, commit }, payload) {
		const toDispatch = []
		const clonedDocs = []
		for (const doc of payload.results) {
			...
			toDispatch.push({
				getChildrenToClone: {
					originalNode: payload.originalNode,
					cloneProductId: payload.cloneProductId,
					parentId: doc._id,
					cloneParentId: clonedDoc._id,
					cloneParentNode: clonedNode,
				},
			})
		}
	...
	getChildrenToClone({ rootState, dispatch }, payload) {
		globalAxios({
			method: 'GET',
			...
			.then((res) => {
				...
				if (results.length > 0) {
					...
					dispatch('processItemsToClone', {
						...
						cloneParentId: payload.cloneParentId,
						cloneParentNode: payload.cloneParentNode,
					})
				} else {
	...
	addHistToClonedDoc({ rootState, dispatch }, payload) {
		globalAxios({
			method: 'GET',
			...
			.then((res) => {
				...
				if (payload.cloneProductId) {
					...
					dispatch('assignProductToUserAction', {
						dbName: rootState.userData.currentDb,
						selectedUser: rootState.userData.user,
						newProductOption,
						userRoles,
						onFailureCallback: () => {
							busyCloning = false
	...
	showBranch({ rootState, commit }, payload) {
		...
		commit('addToEventList', {
			txt: `The ${rootState.helpersRef.getLevelText(payload.originalNode.level)} '${payload.originalNode.title}' and ${clonedDocsCount - 1} descendants are cloned`,
			severity: SEV.INFO,
		})
		busyCloning = false
	},
