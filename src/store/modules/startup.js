...
const actions = {
  ...
  getOtherUserData ({
    ...
    globalAxios({
      ...
      for (const name of Object.keys(allUserData.myDatabases)) {
      ...
      dispatch('watchdog')
      const msg = "getOtherUserData: '" + rootState.userData.user + "' has logged in"
      // eslint-disable-next-line no-console
      if (rootState.debug) console.log(msg)
  ...
  getAllProducts ({
    ...
    globalAxios({
      ...
      for (const p of payload.currentDbSettings.subscriptions) {
      ...
      for (const id of Object.keys(payload.currentDbSettings.productsRoles)) {
