<template>
	<div class="story-column-item" @contextmenu.prevent="showContextMenu = !showContextMenu">
		<div>
			...
			<p class="small-text">Feature: {{ story.featureName }}</p>
		</div>
	</div>
	<BModal v-model="showContextMenu" size="sm" :ok-disabled="disableOkButton" @ok="procSelected" @cancel="doCancel" title="User story menu">
		<BListGroup>
			...
			<template v-if="onLargeScreen">
				<BListGroupItem button :active="contextOptionSelected === VIEW_STORY_IN_TREE" variant="dark" @click="prepSelected(VIEW_STORY_IN_TREE)"
					>View this {{ storyType }} in the tree backlog view</BListGroupItem
				>
			</template>
			<BListGroupItem button :active="contextOptionSelected === REMOVE_STORY" variant="danger" @click="prepSelected(REMOVE_STORY)"
				>Remove this {{ storyType }} from the sprint</BListGroupItem
			>
		</BListGroup>
	</BModal>
</template>
