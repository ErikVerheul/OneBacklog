	...
	<div :class="getClass(columnName)" @contextmenu.prevent="showContextMenu = !showContextMenu">
		...
		<BModal v-model="showContextMenu" size="sm" :ok-disabled="disableOkButton" @ok="procSelected" @cancel="doCancel" title="Task menu">
			<BListGroup>
				...
				<BListGroupItem button :active="ctxOptSelected === CHANGE_TITLE" variant="dark" @click="prepSelected(CHANGE_TITLE)">Change task title </BListGroupItem>
				<BListGroupItem button :active="ctxOptSelected === CHANGE_OWNER" variant="dark" @click="prepSelected(CHANGE_OWNER)">Change task owner </BListGroupItem>
				<BListGroupItem button :active="ctxOptSelected === ID_TO_CLIPBOARD" variant="dark" @click="prepSelected(ID_TO_CLIPBOARD)">Copy short id to clipboard </BListGroupItem>
				<template v-if="onLargeScreen">
					<BListGroupItem button :active="ctxOptSelected === VIEW_TASK_IN_TREE" variant="dark" @click="prepSelected(VIEW_TASK_IN_TREE)"
						>View task in backlog tree view
					</BListGroupItem>
				</template>
				<BListGroupItem button :active="ctxOptSelected === REMOVE_TASK" variant="danger" @click="prepSelected(REMOVE_TASK)">Remove this task </BListGroupItem>
			</BListGroup>

			<div v-if="ctxOptSelected === ADD_TASK" class="title_block">
				<BFormInput v-model="newTaskTitle" placeholder="Enter the title of the new task" />
			</div>

			<div v-if="ctxOptSelected === CHANGE_TITLE" class="title_block">
				<BFormInput v-model="changedTaskTitle" placeholder="Change the title of this task" />
			</div>
			...
			<div v-if="ctxOptSelected === CHANGE_OWNER" class="title_block">
				<BRow class="my-1">
					...
					<BCol sm="6">
						<BFormGroup>
							<BFormSelect v-model="selectedUser" :options="userOptions" />
						</BFormGroup>
					</BCol>
				</BRow>
			</div>
