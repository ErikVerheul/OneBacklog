	...
	<BModal ref="contextMenuRef" :ok-disabled="disableOkButton" @ok="procSelected" @cancel="doCancel" :title="contextNodeTitle">
		<template v-if="contextOptionSelected === SHOWDEPENDENCIES || contextOptionSelected === SHOWCONDITIONS">
			<template v-if="contextOptionSelected === SHOWDEPENDENCIES">
				...
				<p class="my-4">
					<b>Select the items that are no longer dependent on this {{ contextNodeType }}</b>
				</p>
			</template>
			<template v-if="contextOptionSelected === SHOWCONDITIONS">
				...
				<p class="my-4">
					<b>Select the items that are no longer conditional for this {{ contextNodeType }}</b>
				</p>
			</template>
		</template>
		<template v-else>
				...
				<template v-if="!store.state.moveOngoing && !store.state.selectNodeOngoing">
					<template v-if="isAPO && isReqAreaItem">
						<template v-if="isReqAreaTopLevel">
							<BListGroupItem button :active="contextOptionSelected === INSERTINSIDE" variant="dark" @click="showSelected(INSERTINSIDE)"
								>Insert a requirement area inside this node</BListGroupItem
							>
						</template>
						<template v-else>
							<BListGroupItem button :active="contextOptionSelected === INSERTBELOW" variant="dark" @click="showSelected(INSERTBELOW)"
					...
					<template v-else-if="(isPO || isDeveloper) && contextNodeLevel > EPICLEVEL && contextNodeTeam !== myTeam">
						<BListGroupItem v-if="myTeam === NOTEAM" button :active="contextOptionSelected === ASSIGNTOMYTEAM" variant="dark" @click="showSelected(ASSIGNTOMYTEAM)"
							>Join team '{{ contextNodeTeam }}' to open the context menu here</BListGroupItem
						>
						<BListGroupItem v-else-if="contextNodeLevel > USLEVEL" button :active="contextOptionSelected === ASSIGNTOMYTEAM" variant="dark" @click="showSelected(ASSIGNTOMYTEAM)"
							>Assign this {{ contextNodeType }} to my team</BListGroupItem
						>
						<BListGroupItem v-else button :active="contextOptionSelected === ASSIGNTOMYTEAM" variant="dark" @click="showSelected(ASSIGNTOMYTEAM)"
							>Assign this {{ contextNodeType }} and its {{ contextNodeDescendants.count }} descendants to my team</BListGroupItem
						>
					</template>
					<template v-else>
						<!-- cannot create a database or product here -->
						<BListGroupItem v-if="contextNodeLevel > PRODUCTLEVEL" button :active="contextOptionSelected === INSERTBELOW" variant="dark" @click="showSelected(INSERTBELOW)"
							>Insert a {{ contextNodeType }} below this {{ contextNodeType }}</BListGroupItem
						>
						<!-- cannot create item inside task -->
						<BListGroupItem v-if="contextNodeLevel < TASKLEVEL" button :active="contextOptionSelected === INSERTINSIDE" variant="dark" @click="showSelected(INSERTINSIDE)"
						...
						<BListGroupItem
							...
							>▲ Show/remove existing dependencies on this item
						</BListGroupItem>
						...
						<BListGroupItem
							...
							>▼ Show/remove existing conditions for other items
						</BListGroupItem>
						...
						<BListGroupItem
							...
							>Make a clone of this {{ contextNodeType }}
						</BListGroupItem>
						...
						<BListGroupItem
							...
							>Make a copy of this {{ contextNodeType }}
						</BListGroupItem>
						...
						<BListGroupItem v-if="canAssignUsToSprint" button :active="contextOptionSelected === USTOSPRINT" variant="dark" @click="showSelected(USTOSPRINT)">
							Assign this {{ contextNodeType }} to a sprint</BListGroupItem
						>

						<BListGroupItem v-if="canAssignTaskToSprint" button :active="contextOptionSelected === TASKTOSPRINT" variant="dark" @click="showSelected(TASKTOSPRINT)"
							>Assign this task to a sprint</BListGroupItem
						>

						<BListGroupItem v-if="isInSprint" button :active="contextOptionSelected === FROMSPRINT" variant="dark" @click="showSelected(FROMSPRINT)"
							>Remove this {{ contextNodeType }} from the sprint</BListGroupItem
						>
						...
						<BListGroupItem
							...
							>Delete this {{ contextNodeType }} and its {{ contextNodeDescendants.count }} descendants</BListGroupItem
						>
					</template>
				</template>

				<BListGroupItem v-if="store.state.selectNodeOngoing" button :active="contextOptionSelected === SETDEPENDENCY" variant="dark" @click="showSelected(SETDEPENDENCY)"
			...
			<div class="text-center">
				...
				<BButton v-if="contextOptionSelected !== undefined" v-show="!showAssistance" size="sm" variant="seablue" @click="showAssistance = 'true'">Need assistance?</BButton>
				<div v-if="showAssistance" class="assist-text" v-html="assistanceText" />
