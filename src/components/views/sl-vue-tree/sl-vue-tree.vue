	...
	<div class="sl-vue-tree" :class="{ 'sl-vue-tree-root': isRoot }" @mousemove="mouseMoveHandler">
		...
		>
			<div
				...
				:style="{
					visibility: cursorPosition && cursorPosition.nodeModel.pathStr === node.pathStr && cursorPosition.placement === 'before' ? 'visible' : 'hidden',
				}"
			/>

			<template
				class="sl-vue-tree-node-item"
				@mousedown.left="mouseDownLeftHandler($event, node)"
				@mouseup.left="mouseUpLeftHandler($event, node)"
				@contextmenu="contextMenuHandler($event, node)"
				:path="node.pathStr"
			>
				<div
					:class="{
						'sl-vue-tree-cursor-inside': cursorPosition && cursorPosition.placement === 'inside' && cursorPosition.nodeModel.pathStr === node.pathStr,
					}"
				>
					...
					<div class="sl-vue-tree-dependencyviolation">
						<slot name="dependencyviolation" :node="node" />
					</div>
...
.sl-vue-tree {
	...
	&-selected > .sl-vue-tree-node-item {
		background-color: #13242d;
		color: white;
	}

	&-highlighted-1 > .sl-vue-tree-node-item {
		color: rgb(98, 153, 226);
	}

	&-highlighted-2 > .sl-vue-tree-node-item {
		color: rgb(99, 233, 122);
	}

	&-warnlighted > .sl-vue-tree-node-item {
		color: red;
	}
	...
	.sl-vue-tree-node-item {
		...
		display: flex;
		flex-direction: row;
		padding: 0 10px; // Combined padding-left and padding-right
		line-height: 27px;
		user-select: none;
	}
