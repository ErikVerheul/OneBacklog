          ...
          <b-col cols="3">
            <h3 v-if="$store.state.currentDoc._id !== 'root'" align="right">
              ...
              <b-dropdown v-if="$store.state.currentDoc.level < LEVEL.TASK" right class="m-1 .btn.btn-secondary.dropdown-toggle">
                <template v-slot:button-content>{{ getItemStateText($store.state.currentDoc.state) }}</template>
              ...
              <b-dropdown v-else right class="m-2 .btn.btn-secondary.dropdown-toggle">
                <template v-slot:button-content>{{ getTaskStateText($store.state.currentDoc.state) }}</template>
      ...
      <div class="pane" :style="{ minWidth: '30%', width: '50%', minHeight: '100%' }">
        ...
        <div class="tree-container" @mousedown.stop>
          <sl-vue-tree :value="$store.state.treeNodes" @nodes-are-selected="onNodesSelected" @beforedrop="beforeNodeDropped" @drop="nodeDropped">
            ...
            <template v-if="$store.state.userData.myOptions.proUser === 'true' && node.tmp.markedViolations" v-slot:dependency-violation="{ node }">
