<template>
  <div>
    <b-modal size="lg" ref="myFiltersRef" @ok="onApplyMyFilters" title="View, set and/or save your filters">
      <b-container fluid align-v="true">
        <b-row class="my-1">
          <b-col sm="12" v-if="$store.state.userData.myOptions.proUser === 'true'">
          ...
          <b-col sm="12" v-if="$store.state.userData.myOptions.proUser === 'true'">
            ...
            <div v-if="filterOnReqAreas" class="indent20">
              <b-form-group>
                <b-form-checkbox-group v-model="selectedReqAreas" :options="this.$store.state.reqAreaOptions"
                  value-field="id" text-field="title" stacked></b-form-checkbox-group>
              </b-form-group>
            </div>
            <hr />
          </b-col>
