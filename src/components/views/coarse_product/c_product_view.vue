  ...
  <div>
    <app-header>
      ...
      <b-navbar-nav class="ml-auto">
        <b-nav-form>
          <b-button class="m-1" v-show="$store.state.c_changeHistory.length > 0 && !$store.state.busyRemoving" @click="onUndoEvent()">Undo</b-button>
          <b-button class="m-1" @click="onSetMyFilters()">{{ $store.state.c_filterText }}</b-button>
          <div class="divider"/>
          <b-input-group>
            <b-form-input id="findItemOnId" v-model="$store.state.c_shortId" placeholder="Find on Id"></b-form-input>
          ...
          <div class="divider"/>
          <b-input-group>
            <b-form-input id="searchInput" v-model="$store.state.c_keyword" placeholder="Search in titles"></b-form-input>
            <b-input-group-append>
