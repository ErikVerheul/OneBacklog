	...
	<div style="height: 80vh; overflow-y: auto">
		<div v-if="selectedForView === 'comments'">
			<div style="width: 98%" v-for="comment in getFilteredComments" :key="comment.timestamp">
				<BCard border-variant="dark" :header="mkCommentHeader(comment)" header-bg-variant="dark" header-text-variant="white" footer-tag="footer" align="center">
					<BCardBody class="list-body">
						<BRow v-if="(isMyAddition(comment, 'addCommentEvent') || isMyAddition(comment, 'replaceCommentEvent')) && !otherUserReactedAfterMe(comment, getFilteredComments)">
							<BCol cols="11">
								<div v-html="prepCommentsText(getEventName(comment), getEventValue(comment), comment.encoding)" />
							</BCol>
							<BCol cols="1">
								<font-awesome-icon icon="edit" @click="startEditMyComment(comment)" />
							</BCol>
						</BRow>
						<div v-else v-html="prepCommentsText(getEventName(comment), getEventValue(comment), comment.encoding)" />
					</BCardBody>
					<template #footer>
		...
		<ul v-else-if="selectedForView === 'attachments'">
			...
			<div v-for="(attach, index) in getAttachments" :key="attach.title + attach.data.digest">
				<span>
					<template v-if="getNrOfTitles() > 1"> {{ index + 1 }}/{{ getNrOfTitles() }} </template>
		...
		<div v-else-if="selectedForView === 'history'">
			<div style="width: 98%" v-for="histItem in getFilteredHistory" :key="histItem.timestamp">
				<BCard border-variant="dark" :header="mkHistHeader(histItem)" header-bg-variant="dark" header-text-variant="white" footer-tag="footer" align="center">
					<BCardBody class="list-body">
						<div v-html="prepHistoryText(getEventName(histItem), getEventValue(histItem), histItem.encoding || null)" />
					</BCardBody>
					<template #footer>
		...
		<div v-else-if="selectedForView === 'messages'">
			<div style="width: 98%" v-for="msgEvent in store.state.teamMessages" :key="msgEvent.timestamp">
				<BCard border-variant="dark" :header="msgEvent.title" header-bg-variant="dark" header-text-variant="white" footer-tag="footer" align="center">
					<BCardBody class="list-body">
						<BRow v-if="(isMyAddition(msgEvent, 'teamMessage') || isMyAddition(msgEvent, 'replacedTeamMessage')) && !otherUserReactedAfterMe(msgEvent, store.state.teamMessages)">
							<BCol cols="11">
								<div v-html="getMsgContent(msgEvent)" />
							</BCol>
							<BCol cols="1">
								<font-awesome-icon icon="edit" @click="startEditMyMessText(msgEvent)" />
							</BCol>
						</BRow>
						<div v-else v-html="getMsgContent(msgEvent)" />
					</BCardBody>
					<template #footer>
