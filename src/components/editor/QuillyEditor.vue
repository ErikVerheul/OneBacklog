/* * This code originates from https://github.com/alekswebnet/vue-quilly and is adapted to not emit a blur event. * The original blur was not emitted when selecting a non input
element outside the editor. * The original code is licensed under MIT License. */
...
watch(
	...
	(newValue) => {
		...
		if (newValue && newValue !== model.value) {
			...
			model.value = props.isSemanticHtmlModel ? quillInstance.getSemanticHTML() : quillInstance.root.innerHTML
		} else if (!newValue) {
