			...
			<template v-else>
				...
				<template v-else-if="getUserFirst">
					...
					<BRow class="my-1">
						...
						<BCol sm="3">
							<BFormGroup>
								<BFormSelect v-model="selectedUser" :options="store.state.userOptions" />
							</BFormGroup>
						</BCol>
					</BRow>
					...
					<div v-if="optionSelected === 'Remove a user'">
						<div v-if="store.state.isUserFound && !store.state.isUserDeleted">
							<hr />
							<BButton class="m-1 btn btn-danger" @click="doRemoveUser" variant="primary">Remove user '{{ selectedUser }}' </BButton>
							<BButton class="m-1" @click="cancel">Cancel</BButton>
						</div>
							...
							<BRow class="my-1">
								...
								<div v-if="!isUserDbSelected">
									<BCol sm="12">
										<BFormGroup>
											...
											<p>
												The databases {{ getUserAssignedDatabases() }} are assigned to this user. Assigning roles to products in another database will also assign that database.
											</p>
								...
								<div v-else>
									<BCol sm="12">
										...
										<p>
											The admin role is a generic role with access to all user profiles and all product definitions in this database<br />
											The APO role manages requirement areas and can prioritize features
										</p>
										<BFormGroup>
										...
										<p>
											The assistant admin role is a generic role with access to all user profiles and all product definitions in this database. However, the assistant admin can
											only create users for databases/products he/she is assigned to by an admin.
										</p>
									...
									<BCol sm="12">
										<div v-if="store.state.areProductsFound">
											...
											<div v-for="prod of store.state.useracc.dbProducts" :key="prod.id">
												{{ prod.value }}:
												<BFormGroup>
													<BFormCheckboxGroup v-model="prod.roles" :options="roleOptions" />
												</BFormGroup>
											</div>
											<hr />
											<p v-if="!canRemoveLastProduct">
												You cannot remove the last role of the last assigned product in the only database of this user. Consider the option to remove this user.
											</p>
				...
				<template v-else>
					<div v-if="!dbIsSelected">
						<BFormGroup>
							...
							<p v-if="optionSelected === 'Create a product'">
								Note: The product will be assigned to your profile only. Use 'Maintain user permissions to products' as a next step to assign the product to other users.
							</p>
							<BFormRadioGroup v-model="store.state.selectedDatabaseName" :options="store.state.databaseOptions" stacked />
						</BFormGroup>
						<hr />
					...
					<div v-else>
						<template v-if="optionSelected === 'Create a user and assign product(s)'">
							<div v-if="!credentialsReady">
								...
								<BRow>
									<BCol sm="2"> User name: </BCol>
									<BCol sm="10">
										<BFormInput v-model="userName" placeholder="Enter the user name" />
									</BCol>
									<BCol sm="2"> Initial password: </BCol>
									<BCol sm="10">
										<BFormInput v-model="password" type="password" placeholder="Enter the initial password. The user must change it." />
									</BCol>
									<BCol sm="2"> Users e-mail: </BCol>
									<BCol sm="10">
										<BFormInput v-model="userEmail" type="email" placeholder="Enter the user's e-mail" />
									</BCol>
									<hr />
									<BCol sm="12">
							...
							<div v-if="credentialsReady">
								<div v-if="store.state.areProductsFound">
									...
									<BFormCheckbox v-model="store.state.useracc.userIsAdmin">Tick to add this role </BFormCheckbox>
									<h5>Make this user an 'APO'?</h5>
									<BFormCheckbox v-model="store.state.useracc.userIsAPO">Tick to add this role </BFormCheckbox>
									<hr />
						...
						<template v-else-if="optionSelected === 'Create a product'">
							...
							<BFormInput v-model="productTitle" placeholder="Enter the product title" />
						...
						<template v-else-if="optionSelected === 'Create a team'">
							...
							<BFormInput v-model="teamName" placeholder="Enter the team name" />
						...
						<template v-else-if="optionSelected === 'Remove teams without members'">
							...
							<BFormGroup label="Select one or more teams to remove">
								<BFormCheckboxGroup v-model="teamNamesToRemove" :options="teamsToRemoveOptions" />
							</BFormGroup>
						...
						<template v-if="optionSelected === 'Maintain the default sprint calendar' || optionSelected === 'Create / Maintain a team sprint calendar'">
							...
							<template v-if="store.state.isDefaultCalendarLoaded || store.state.isTeamCalendarLoaded">
								...
								<BListGroup>
									<BListGroupItem button variant="secondary" v-b-modal.modal-extend>Extend this calendar with new sprints</BListGroupItem>
									<BListGroupItem button variant="secondary" v-b-modal.modal-change>Change a sprint length and shift all its successors in time</BListGroupItem>
								</BListGroup>
								<BButton class="m-3" @click="cancel()">Return</BButton>
								<BModal @ok="extendCalendar" id="modal-extend" :ok-disabled="extendDisableOkButton" title="Extend the number of sprints">
									<BFormInput v-model="extendNumberStr" type="number" placeholder="Enter the number of extensions" />
								</BModal>
								<BModal @ok="changeSprintInCalendar" id="modal-change" :ok-disabled="changeDisableOkButton" title="Change a sprint duration" size="lg">
									<BContainer>
										<BRow class="mb-1">
											<BCol cols="3"> Sprint number </BCol>
											<BCol cols="9">
												<BFormInput v-model="changedNumberStr" type="number" placeholder="Enter the sprint number" />
											</BCol>
										</BRow>
										{{ acceptSprintNrMsg }}
										<div v-if="acceptSprintnr">
											<BRow class="mb-1">
												<BCol cols="3"> Selected </BCol>
												<BCol cols="9"> sprint-{{ changedNumberStr }} </BCol>
												<BCol cols="3"> Starting </BCol>
												<BCol cols="9">
													{{ getStartDate() }}
												</BCol>
												<BCol cols="3"> Duration </BCol>
												<BCol cols="9"> {{ getDuration() }} days </BCol>
												<BCol cols="3"> Ending </BCol>
												<BCol cols="9">
													{{ getEndDate() }}
												</BCol>
												<BCol cols="3"> New duration </BCol>
												<BCol cols="9">
													<BFormInput v-model="changedDurationStr" type="number" placeholder="Enter a new duration in days (1-28)" />
												</BCol>
												<BCol cols="3"> Hours shift </BCol>
												<BCol cols="9">
													<BFormInput v-model="changedHourStr" type="number" placeholder="Shift the sprint ending time (-12,+12) hours" />
												</BCol>
											</BRow>
										</div>
										<div v-if="acceptSprintnr && acceptNewSprintLength && acceptHourChange && acceptNewEndDate">
											<BRow class="mb-1">
												<BCol cols="12"> Your changes: </BCol>
											</BRow>
											<BRow class="mb-1">
												<BCol cols="2"> Duration </BCol>
												<BCol cols="10"> {{ changedDurationStr }} days </BCol>
											</BRow>
											<BRow class="mb-1">
												<BCol cols="2"> Ending </BCol>
												<BCol cols="10">
													{{ calcNewEndDate() }}
												</BCol>
											</BRow>
						...
						<template v-else-if="optionSelected === 'Set the history retention paramaters'">
							...
							<BFormInput v-model="maxHistoryDays" />
							<p>Enter maximum number of history items to keep:</p>
							<BFormInput v-model="maxHistoryEvents" />
							<BButton v-if="maxHistoryDays > 0 && maxHistoryEvents > 0" class="m-3" @click="doSaveHistoryRetentionSettings" variant="primary"> Save</BButton>
							<BButton class="m-3" @click="cancel">Cancel</BButton>
						</template>
					</div>
				</template>
