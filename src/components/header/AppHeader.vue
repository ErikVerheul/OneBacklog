<template>
	<template v-if="onLargeScreen">
		<BNavbar toggleable="lg" class="navbar-dark bg-dark">
			<BNavbar-toggle target="nav_collapse" />
			...
			<BCollapse is-nav id="nav_collapse">
				...
				<BNavbarNav v-if="store.state.showHeaderDropDowns" class="ms-auto">
					...
					<BNavItemDropdown text="Select your view" right>
						...
						<BDropdownDivider v-if="isAssistAdmin || isAdmin || isServerAdmin" />
				...
				<BCard bg-variant="light">
					...
					<form @submit.prevent="doChangeMyPassWord">
						<input
							...
							:aria-label="'Username'"
							:value="store.state.userData.user"
							style="position: absolute; left: -9999px; width: 1px; height: 1px; overflow: hidden; border: 0; padding: 0; margin: 0"
							tabindex="-1"
							readonly
						/>
						<BFormGroup label-cols-lg="5" label="The new password must have 8 or more characters" label-size="lg" label-class="font-weight-bold pt-0" class="mb-0">
							<BFormGroup label-cols-sm="5" label="Current password:" label-align-sm="right">
								...
								<BFormInput v-model="oldPassword" type="password" autocomplete="new-password" />
							</BFormGroup>
							<BFormGroup label-cols-sm="5" label="New password:" label-align-sm="right">
								<BFormInput v-model="newPassword1" type="password" autocomplete="new-password" />
							</BFormGroup>
							<BFormGroup label-cols-sm="5" label="Retype new password:" label-align-sm="right">
								<BFormInput v-model="newPassword2" type="password" autocomplete="new-password" />
							</BFormGroup>
						</BFormGroup>
				...
				<BCard bg-variant="light">
					<BFormGroup label-cols-lg="5" label="Change notices are send to this email address" label-size="lg" label-class="font-weight-bold pt-0" class="mb-0">
						...
						<BFormGroup label-cols-sm="5" label="New email:" label-align-sm="right">
							<BFormInput v-model="newEmail1" type="email" />
						</BFormGroup>
						<BFormGroup label-cols-sm="5" label="Retype new email:" label-align-sm="right">
							<BFormInput v-model="newEmail2" type="email" />
						</BFormGroup>
					</BFormGroup>
				</BCard>
			</BRow>
	...
	<BModal size="lg" ref="showMyRolesRef" title="My authorizations">
		<BContainer align-v="true">
			<h3>Generic roles :</h3>
			<p>
				By default the application uses two databases. The _users database owned by the admin role and a database holding the products. More databases can be created but the _users
				database is shared.<br />
				What a user can see or do is determined by the roles assigned to that user.
			</p>
			<ul>
				<li>
					'_admin': One user with this role is the CouchDb Server administrator with 'root' rights. This user can setup, replicate and delete databases. See the CouchDB
					documentation. At application init this user also becomes the role of database admin (see below).
				</li>
			</ul>
			...
			<ul>
				<li>
					'admin' or 'database admin': Can create products, teams and users. Can (un)assign databases and roles to users and user access to products. Need not be a member of a
					team.
				</li>
				<li>
					'assistAdmin': An admin can delegate tasks to assistant admins as an extension to their product specific roles (see below) and only for the databases and products
					assigned to them. Can create teams and users. Can (un)assign databases and products to users. Can (un)assign user roles per product. Cannot (un)assign global roles or
					create products or remove users. Need not be a member of a team.
				</li>
				<li>'APO': The Area Product Owners create and maintain their requirement areas. Can change priorities at the epic and feature level. Is not member of a team.</li>
			</ul>
			...
			<ul>
				<li>
					'PO': Maintains product definitions, creates and maintains epics, features and user stories for the assigned products. Can change priorities at these levels. Must be
					member of a team.
				</li>
				<li>'developer': Can create and maintain user stories and features for the assigned products when team member. Must be member of a team.</li>
	...
	<BModal size="lg" v-if="store.state.userData.myOptions" v-model="showOptionsModal" no-footer title="Options menu">
		...
		<BFormCheckbox v-model="store.state.userData.myOptions.proUser" value="true" unchecked-value="false"> Use the professional mode of this app </BFormCheckbox>
		...
		<template v-if="store.state.userData.user !== 'demoUser'">
			...
			<BFormCheckbox v-model="store.state.userData.myOptions.doNotMessageMyself" value="true" unchecked-value="false"> Do not send change notices to myself </BFormCheckbox>
		</template>
