		...
		<BContainer>
			...
			<div v-if="optionSelected === 'View the log'">
				...
				<BRow>
					...
					<BCol>
						...
						<div>
							<BFormCheckboxGroup v-model="selectedLogLevels" :options="options" stacked class="mb-3" value-field="item" text-field="name" disabled-field="notEnabled" />
						</div>
					</BCol>
				</BRow>
				<hr />
				...
				<BModal scrollable v-model="showLogModal" size="lg" :title="logModalTitle()">
					<div v-for="item in filtered(store.state.logEntries)" :key="createLogKey(item.timestamp, item.sessionId, item.sessionSeq)">
						...
						<template v-if="item.saveTime"> <br />Save Time: {{ new Date(item.saveTime) }} </template>
						<hr />
					</div>
				</BModal>
			</div>
			...
			<div v-if="optionSelected === 'Restore a database from backup'">
				...
				<template v-if="store.state.selectedDatabaseName !== 'not selected yet'">
					<p class="colorRed">
						Database '{{ dbToReplace }}' will be replaced by '{{ store.state.selectedDatabaseName }}'. Make sure no users of this database are on-line right now.
					</p>
					<p class="colorRed" v-if="isCurrentDbSelected()">You are replacing your current database. When the restore is ready, you will be signed-out automatically.</p>
				</template>
				<hr />
				<BButton v-if="store.state.selectedDatabaseName !== 'not selected yet' && !store.state.utils.copyBusy" class="m-1" @click="doRestoreBackup" variant="primary">
			...
			<div v-if="optionSelected === 'Create a new database'">
				...
				<BFormInput v-model="newDbName" placeholder="Enter the database name" />
				<hr />
				<BButton v-if="newDbName === ''" class="m-1" @click="cancel">Cancel</BButton>
				<div v-else>
					<p>Database {{ newDbName }} will be created</p>
					<BButton v-if="!store.state.isDatabaseCreated" class="m-1" @click="doCreateDatabase" variant="primary">Start creation</BButton>
			...
			<div v-if="optionSelected === 'Purge removed documents and compact the database'">
				...
				<hr />
				<BButton v-if="!store.state.isPurgeReady" class="m-1" @click="doPurgeDb" variant="primary">Purge removed documents and compact the database</BButton>
			...
			<div v-if="optionSelected === 'Change my default database to any available database'">
				...
				<hr />
				<BButton v-if="!store.state.isCurrentDbChanged" class="m-1" @click="doChangeMyDb" variant="primary">Change my database</BButton>
				<BButton v-if="!store.state.isCurrentDbChanged" class="m-1" @click="cancel">Cancel</BButton>
				<BButton v-else class="m-1" @click="cancel" variant="primary">Return</BButton>
				<div v-if="store.state.isCurrentDbChanged">
					<h4>Success! Click 'Exit' to sign-out. Sign-in to see the Backlog tree view of the '{{ store.state.selectedDatabaseName }} 'database</h4>
					<div>
			...
			<div v-if="optionSelected === 'Remove old comments on age'">
				...
				<BRow class="my-1">
					<BCol sm="2"> Remove when created </BCol>
					<BCol sm="2">
						<BFormInput v-model="removeAge" type="number" />
					</BCol>
					<BCol sm="8"> days ago or more (enter 0 to remove all) </BCol>
				</BRow>
				<hr />
			...
			<div v-if="optionSelected === 'Delete a database'">
				...
				<hr />
				<BButton v-if="store.state.selectedDatabaseName" variant="danger" class="m-1" @click="doDeleteDb">Delete selected database</BButton>
